...
#+begin_challenge
{{{markdown(## Challenge: Shadowing the Standard Library)}}}
...
#+BEGIN_SRC dot :file ../../episodes/fig/python-import-search-path.png :cmdline -Kdot -Tpng -Gdpi=150 :cache yes :exports none
digraph ImportLogic {
    rankdir=TD;
    node [fontname="Helvetica", style=filled];
    edge [fontname="Helvetica", fontsize=10];

    start [label="import my_module", shape=invhouse, fillcolor="#d1c4e9"];

    subgraph cluster_path {
        label="sys.path Search Order";
        style=dashed;
        color="#757575";
        fontcolor="#757575";
        margin=20;

        cwd [label="1. Current Directory\n('')", shape=box, fillcolor="#ffccbc"];
        stdlib [label="2. Standard Library\n(/usr/lib/...)", shape=box, fillcolor="#b3e5fc"];
        site [label="3. Site Packages\n(.../site-packages)", shape=box, fillcolor="#c8e6c9"];
    }

    success [label="Module Loaded", shape=box, style="rounded,filled", fillcolor="#dcedc8", fontcolor="#33691e"];
    fail [label="ModuleNotFoundError", shape=octagon, fillcolor="#ffcdd2", fontcolor="#b71c1c"];

    start -> cwd;
    cwd -> success [label="Found"];
    cwd -> stdlib [label="Not Found"];
    stdlib -> success [label="Found"];
    stdlib -> site [label="Not Found"];
    site -> success [label="Found"];
    site -> fail [label="Not Found"];
}
#+END_SRC

#+RESULTS[f61c12f9db08f20e8a6a27cda51290890e345782]:

@@markdown:![Search order for packages](fig/python-import-search-path.png)@@
...
It is the first file executed when you import the package. It can be empty, but
it is often used to expose functions to the top level.
...
#+begin_challenge
{{{markdown(## Challenge: Moving Directories)}}}
...
*Reason:*
You moved out of the folder containing =chemlib=. Since the package is not
installed in the global =site-packages=, and the current directory no longer
contains it, Python's search through =sys.path= fails to find it.
#+end_solution
#+end_challenge

#+BEGIN_SRC dot :file ../../episodes/fig/python-package-structure.png :cmdline -Kdot -Tpng -Gdpi=150 :cache yes :exports none :results none
digraph PackageTree {
    rankdir=TB;
    node [shape=note, fontname="Helvetica", style=filled, fillcolor="#ffffff", width=1.8];
    ...
    subgraph cluster_project {
        ...
        script [label="script.py\n(Script)", fillcolor="#fff9c4"]; // Yellow

        subgraph cluster_pkg {
            label="chemlib/\n(Package)";
            style=filled;
            fillcolor="#ffe0b2"; // Orange
            margin=20;

            init [label="__init__.py\n(Package Marker)", fillcolor="#ffccbc"]; // Deep Orange
            geo [label="geometry.py\n(Module)"];
        }
    }
}
#+END_SRC
...
@@markdown:![Directory structure for current setup](fig/python-package-structure.png)@@

To solve this, we need a standard way to tell Python "This package exists,
please add it to your search path permanently." This is the job of *Packaging*
and *Installation*.
